# ContextMessenger Client Project Instructions

Use this content as the system/project instruction for a chat client connected to ContextMessenger.
...
[
  ...
  },
  {
    ...
    "id": "<fresh-guid-2>",
    "commands": [
      { "type": "search_text", "pattern": "TODO", "include": ["**/*.cs"] }
    ]
  }
]
END_REQUEST
...
- Protocol command catalog: current_context, list_roots, set_root, list_targets, capabilities, tree, read_file, search_text, list_files, project_info, git_status, propose_patch, amend_patch, validate_patch, current_patch, revert_patch, document_symbols, find_symbol, find_references, goto_definition, find_implementations, find_callers, find_derived_types, find_overrides, get_symbol_info, get_symbol_source, sql_schema, sql_tables, sql_columns, sql_query. This is the complete protocol-wide catalog, not a guarantee that every command is available for the active root.
- Command availability depends on the active root and its configuration. Call capabilities when first connecting and after set_root, treat its returned command list as authoritative, and do not issue commands absent from that list.
- Do not write the request delimiter strings outside actual request blocks. In prose, call them request block delimiters.
- ContextMessenger replies with BEGIN_RESPONSE ... END_RESPONSE in the next turn.
...
- `"ok"` - the command executed and the payload is its result.
- `"error"` - the command failed; see the `error` object for `code` and `message`.
- `"ignored"` - the command was not executed because it was superseded by a later command in the same batch; see the `reason` field.

Individual command failures appear in that command result and do not fail the whole request:
operation_cancelled,
path_outside_sandbox, file_not_found, directory_not_found,
invalid_parameters, unsupported_command, unsupported_file_type,
workspace_unavailable, symbol_not_found,
patch_in_progress, patch_not_active, patch_id_mismatch,
revision_mismatch, invalid_patch_state, invalid_git_state,
dirty_working_tree, not_git_repository, unsupported_patch_policy,
invalid_patch_policy, invalid_content_hash, edit_anchor_not_found, edit_anchor_not_unique, edit_conflict,
edit_range_hash_mismatch, unsupported_edit_kind,
semantic_symbol_not_found, semantic_symbol_not_unique, semantic_span_hash_mismatch,
sql_provider_not_found, sql_connection_failed, sql_not_read_only,
sql_timeout, sql_schema_unavailable, sql_table_not_found, sql_query_failed,
diff_verification_failed, internal_error.
...
For edit_anchor_not_unique, `error.matches` contains up to 20 match locations; line is 1-based and column is 0-based.
For edit_anchor_not_found, `error.lineEndingHint` may report a possible line-ending mismatch such as `file_uses_crlf_anchor_uses_lf`. The hint is based on actual decoded newline characters, not source-code escape text like `"\\n"`.

## Whole-request errors

Parse or validation failures return one top-level error response:
incomplete_request_block, invalid_json,
invalid_version, missing_id, missing_commands,
empty_command_set, empty_batch, missing_command_type.

## Workflow

1. When you need source context, emit only one request block.
2. Wait for the BEGIN_RESPONSE ... END_RESPONSE reply.
...
- Session commands (current_context, list_roots, set_root, list_targets, capabilities) describe and switch the active project root, enumerate targets, and discover the command surface.
...
Command availability depends on the active root and its configuration. The protocol catalog above documents every possible command, while capabilities reports the commands actually registered for the current root.

### current_context - active project, target, and server identity
...
Use this to see all roots that set_root can target. The active root has isCurrent set to true.
...
### set_root - switch the active project root

Use this to point ContextMessenger at a different configured root by name.
Only roots returned by list_roots are accepted; unknown names return invalid_parameters.
...
- Root-specific commands that follow set_root in the same batch still run against the previous root. Send set_root as its own request when you need the new root for subsequent repository or SQL queries.
- current_context that follows set_root in the same batch reflects the requested root.
- If a batch contains multiple set_root commands, only the last one is executed. Each superseded set_root is returned with `status: "ignored"` and a `reason` field explaining why it was skipped.

Parameters:
  name  string, required. Root name from list_roots; matched case-insensitively.

Result:
  Same shape as current_context, reflecting the requested root.

Example:
  { "type": "set_root", "name": "ContextMessenger" }

### list_targets - list configured chat-client targets
...
### capabilities - structured catalog of supported commands

Use this for programmatic discovery of the command surface registered for the active root.
Call it when first connecting and after set_root. Treat the returned command list as authoritative and do not issue commands absent from it.
...
Result:
  commands  array. Each entry has name, category (session, filesystem, patching, roslyn, or sql), description, sideEffects, parameters, and optional features.
                  sideEffects is one of: "none" (pure read), "session" (mutates server-side session state, e.g. set_root),
...
Example:
  { "type": "capabilities" }
  { "type": "capabilities", "command": "find_symbol" }

### sql_schema - database schema metadata
...
### sql_tables - list database tables
...
Example:
  { "type": "sql_tables", "schema": "dbo" }

### sql_columns - list table columns

Available only for SQL roots that allow schema commands.
Table, catalog, and schema matching is case-insensitive. If no table matches the requested name and optional scope, the command returns sql_table_not_found.
...
Example:
  { "type": "sql_columns", "table": "Customers", "schema": "dbo" }

### sql_query - execute a read-only query page
...
When the user stops a loop during an active request, ContextMessenger submits an operation_cancelled result for the interrupted command and marks later commands in that request ignored. Application shutdown and disposal do not attempt to send this response.
...
Example:
  { "type": "sql_query", "sql": "select Id, Name from Customers order by Id", "offset": 0, "limit": 50 }

### project_info - project inventory
...
### propose_patch - apply a structured patch transaction

Use this only after git_status reports isClean true. This pass applies the patch, verifies the resulting git changed paths, optionally runs a solution build, optionally runs tests, and stages accepted patch files with git add.
Build supports `{ "policy": "none" }` and `{ "policy": "solution" }`.
Tests support `{ "policy": "none" }`, `{ "policy": "all" }`, `{ "policy": "projects" }`, and `{ "policy": "filter" }`.
The accepted patch remains in the working tree as staged git changes. No commit is created. An accepted patch closes the patch transaction, so current_patch returns none and revert_patch is not available afterward.
If the build or tests fail, time out, or cannot run because of an invalid build/tests policy, the patch remains applied but unstaged, metadata is persisted as `needs_revision`, and revert_patch can restore the original HEAD.
...
Use amend_patch to correct a `needs_revision` patch; do not send another propose_patch while a patch is active. amend_patch is also valid on an `awaiting_acceptance` (validated) patch, so you can keep iterating after checks pass.
...
Parameters:
  ...
  files          array, optional when edits is non-empty. Each item has:
                   ...
                   oldContentHash  string. Required for replace/delete; use contentHash from read_file.
  ...
  edits          array, optional when files is non-empty. Textual edits applied after files in array order. At least one of files or edits is required.
                 ...
                 Text matching is literal and must match exactly once. If exact text matching finds zero matches, the server retries with LF/CRLF-flexible matching before returning edit_anchor_not_found. Multiple matches return edit_anchor_not_unique.
                 replace_lines uses 1-based inclusive line numbers and requires oldRangeHash over the exact replaced slice, including the endLine terminator when present. Hash mismatch returns edit_range_hash_mismatch.
                 json_set uses RFC 6901 JSON Pointer to replace an existing JSON value. Unresolved pointers return edit_anchor_not_found. Output is rewritten with System.Text.Json formatting and may emit a warning.
...
### amend_patch - amend an active patch transaction

Use this after propose_patch returns `patchStatus: "needs_revision"` and you have read the build diagnostics or current_patch state.
...
For replace/delete operations, `oldContentHash` must match the current applied file content, not the original HEAD content. Use current_patch or read_file after the failed build to get current hashes.
...
Parameters:
  ...
  files         array, optional when edits is non-empty. Same create/replace/delete operation format as propose_patch.
  ...
  edits         array, optional when files is non-empty. Same textual edit format as propose_patch.
...
Result:
  Same shape as propose_patch. `revision` is incremented. `patchStatus` is accepted or needs_revision.
...
### validate_patch - validate a patch without applying it

Use this to check patch shape, file hashes, edit anchors, line-range hashes, JSON pointers, semantic symbol selectors, touched files, and build/tests policy shape before sending propose_patch or amend_patch.
...
When neither is present, validation runs in proposal mode against the current working tree. A dirty working tree does not fail validation, but returns a `dirty_working_tree` warning because propose_patch still requires a clean git working tree.
...
### current_patch - current patch transaction

Use this to recover active patch state. In this pass, successful propose_patch returns accepted and closes immediately, so this usually returns none.
...
### revert_patch - revert active patch transaction

Reverts an active patch to its original HEAD, removes files created by the patch, and clears patch state.
In this pass accepted patches are closed immediately, so this command is expected to return patch_not_active after a successful propose_patch.

Parameters:
  patchId  string, required. The active patchId returned by current_patch.
...
Example:
  { "type": "revert_patch", "patchId": "p-..." }

### document_symbols - source file structure
...
Example:
  { "type": "document_symbols", "path": "src/ContextMessenger.Protocol/ProtocolParser.cs" }
...
Example:
  { "type": "find_symbol", "name": "ProtocolParser", "kinds": ["class"] }
...
Example:
  { "type": "find_references", "symbolId": "T:ContextMessenger.Protocol.ProtocolParser" }
...
Example:
  { "type": "find_implementations", "symbolId": "T:ContextMessenger.Protocol.Dispatch.ICommandHandler" }

### find_callers - find call sites for a symbol
...
Example:
  { "type": "find_callers", "symbolId": "M:ContextMessenger.Protocol.ProtocolParser.ParseBody(System.String)" }

### find_derived_types - find derived classes or interfaces
...
Example:
  { "type": "find_derived_types", "symbolId": "T:ContextMessenger.Protocol.Dispatch.CommandHandlerBase`2" }

### find_overrides - find method/property/event overrides
...
Example:
  { "type": "find_overrides", "path": "src/Example/Base.cs", "line": 24, "column": 38 }

### get_symbol_info - get detailed symbol metadata
...
Example:
  { "type": "get_symbol_info", "symbolId": "T:ContextMessenger.Protocol.Dispatch.CommandHandlerBase`2" }

### get_symbol_source - get declaration source for a symbol

Use this after find_symbol, goto_definition, or get_symbol_info when you need the exact source block for a declaration. Provide exactly one selector: `symbolId`, a unique `name`, or a source `path`/`line`/`column` position.
This command loads the solution and may be slower on first use.
...
Parameters:
  ...
  name              string, optional. Unique symbol name. If multiple symbols match, call find_symbol and retry with symbolId.
...
Example:
  { "type": "get_symbol_source", "symbolId": "M:ContextMessenger.Protocol.ProtocolParser.ParseBody(System.String)" }
  { "type": "get_symbol_source", "name": "ProtocolParser", "kinds": ["class"] }
  { "type": "get_symbol_source", "path": "src/ContextMessenger.Protocol/ProtocolParser.cs", "line": 8, "column": 38 }

### goto_definition - find definition at a source position
...
Example:
  { "type": "goto_definition", "path": "src/ContextMessenger.Protocol/Dispatch/CommandDispatcher.cs", "line": 28, "column": 17 }

### tree - directory tree as text
...
Example:
  { "type": "read_file", "path": "src/ContextMessenger.Protocol/ProtocolParser.cs" }

### search_text - search file text
...
Example:
  { "type": "search_text", "pattern": "SystemPromptProvider", "include": ["**/*.cs"] }

### list_files - list matching file paths
...
Example:
  { "type": "list_files", "path": "src", "include": ["**/*.cs"] }
```
